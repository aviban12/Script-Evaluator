...
from train_extractor import clarity_list, coherant_list
...
def store_csv(complete_data,c):
    ...
    my_df.T.to_csv("TestData/datatest{}.csv".format(c),index=False, header=False)
...
def Datatest():
    ...
    """
    test_essay = pd.read_csv("/mnt/1f2870f0-1578-4534-b33f-0817be64aade/projects/Hackerearth/incedo_nlpcadad7d/incedo_participant/test_dataset.csv")
    ...
    for i in essay_Set:
        if i > 0:
            ...
            average_word_length,no_of_word,no_of_sentence= no_of_words(test_essay[set_filter]['EssayText'])
            processed_essay = preprocessdata(test_essay[set_filter]['EssayText'])
            ...
            Flesch_score_list = calculate_Flesch_Score(test_essay[set_filter]['EssayText'])
            ...
            list_column = [count_of_NN,count_of_NNP,count_of_verb,count_of_adverb,flesch_score,
                           ...
                           average_word_length,no_of_word,no_of_sentence]
